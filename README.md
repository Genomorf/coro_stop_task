...
Client deleted
```
# UPDATE 2: то что ниже пока не работает
...
Client deleted
```
То есть так мы можем ждать сообщение от какой-либо сущности, запускать цепочку корутин и в нужный момент, не дожидаясь завершения цепочки, возвращать failed future. Как по мне, это намного проще, чем с gate - меньше кода и сложной логики, все завязано просто на двух фьючах. 

Однако, это не контролируемо. С gate можно отменить только часть нужных нам функций цепочки, а остальные оставить (скорее всего так).
